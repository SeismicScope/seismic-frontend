...
**Theme Tokens** are semantic abstractions designed for dynamic styling. While they map directly to Core Colors, they provide a flexible layer that automatically updates based on the active theme.

**Key Concept**: These tokens are theme-aware, utilizing the `data-theme` attribute to swap values (e.g., Teal, Mango, Tomato) seamlessly across the interface.

## Primary
The Primary token defines the central brand identity and serves as the lead accent color for the user interface.
